...
variable "embedding_model_id" {
  description = "Bedrock Cohere embedding model for knowledge-base retrieval"
  type        = string
...
variable "opensearch_url" {
  description = "OpenSearch endpoint URL reachable from AgentCore Runtime"
  type        = string
}

variable "opensearch_index_name" {
  description = "OpenSearch k-NN index name"
  type        = string
  default     = "nam-documents"
}

variable "kb_search_top_k" {
  description = "Default chunks retrieved per knowledge-base search"
  type        = number
  default     = 5
}
