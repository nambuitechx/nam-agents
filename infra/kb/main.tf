terraform {
  ...
  backend "s3" {
    ...
    key    = "nam-agents/regions/ap-southeast-1/dev/kb/terraform.tfstate"
...
module "runtime" {
  ...
  runtime_slug      = "kb"
  runtime_description = "Knowledge-base Strands agent (Bedrock + OpenSearch) on AgentCore Runtime"
  runtime_module    = "runtimes.knowledge_base"
  ...
  environment_variables = {
    ...
    KB_SEARCH_TOP_K       = tostring(var.kb_search_top_k)
  }
